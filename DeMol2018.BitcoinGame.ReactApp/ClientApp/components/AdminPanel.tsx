...
import {
    ...
    startNewGame,
    startNewRound,
} from "../store/adminPanel/adminPanelSlice";
        ...
        return (
            <div className="adminPanel">
                ...
                <button
                    className="button"
                    onClick={() =>
                        dispatch(startNewGame({ invokerId: playerGuid! }))
                    }
                >
                    Start nieuw spel
                </button>
        ...
        return (
            <div className="adminPanel">
                ...
                <h3>
                    ...
                    {lastRoundNumber == undefined ? "0" : lastRoundNumber}
                </h3>
                <h2>Jokerwinnaars</h2>
                <table className="jokerWinnerTable">
                    <tbody>
                        {jokerWinners!.map((winner, i) => (
                            <tr key={i}>
                                <td className="jokerWinnerName">
                                    {winner.name}
                                </td>
                                <td className="jokerWinnerNumberOfJokersWon">
                                    {winner.numberOfJokersWon} joker
                                    {winner.numberOfJokersWon != 1 ? "s" : ""}
                                </td>
                            </tr>
                        ))}
                    </tbody>
    ...
    return (
        <div className="adminPanel">
            ...
            <button
                className="button"
                onClick={() =>
                    dispatch(finishCurrentGame({ invokerId: playerGuid! }))
                }
            >
                Rond huidig spel af
            </button>
